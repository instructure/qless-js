...
class Client {
  ...
  call() {
    ...
    return this.load()
      ...
      .catch((err) => {
        const context = `qless command "${command}"`;
        const message = err && err.message ? `${err.message} (${context})` : context;
        throw new QlessError(message, err instanceof Error ? err : undefined);
      });
  }
  ...
  load() {
    ...
    return fs.readFileAsync(this.path)
      .catch((err) => {
        throw new QlessError(`Failed to read qless-core scripts at ${this.path}`, err);
      })
